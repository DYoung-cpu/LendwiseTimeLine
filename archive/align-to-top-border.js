...
(async () => {
    ...
    console.log('Loading page to align button to TOP border line...\n');
    ...
    const analysis = await page.evaluate(() => {
        ...
        // Button should be centered on this line
        const buttonHeight = 23;
        const targetButtonCenterY = topBorderLineY;
        const targetButtonTopY = targetButtonCenterY - (buttonHeight / 2);
        ...
        return {
            ...
            targetButtonCenter: targetButtonCenterY,
    ...
    console.log('=== ALIGNMENT TO TOP BORDER LINE ===\n');
    console.log('Border container top:', analysis.borderTop.toFixed(2), 'px');
    console.log('Top border line Y:', analysis.topBorderLineY.toFixed(2), 'px (same as container top)');
    console.log('\nButton specs:');
    console.log('  Height:', analysis.buttonHeight, 'px');
    console.log('  Target center Y:', analysis.targetButtonCenter.toFixed(2), 'px (on top line)');
    ...
    console.log('  Button offset:', analysis.currentBtnOffset.toFixed(2), 'px');
    ...
    const currentSvgY = 58;
    const currentCssTop = 23;
    ...
    // Screenshot before
    await page.locator('.timeline-border-container').screenshot({ path: 'top-border-before.png' });
    console.log('\n✓ Saved top-border-before.png');
    ...
    const verify = await page.evaluate(() => {
        ...
        return {
            ...
            btnOnLine: Math.abs(btnCenterY - topBorderY),
            textInSvg: Math.abs(btnCenterY - svgCenterY),
            success: Math.abs(btnCenterY - topBorderY) < 2 && Math.abs(btnCenterY - svgCenterY) < 2
        };
    });

    console.log('\n=== VERIFICATION ===');
    console.log('Top border line Y:', verify.topBorderY.toFixed(2), 'px');
    console.log('Button center Y:', verify.btnCenterY.toFixed(2), 'px');
    console.log('SVG center Y:', verify.svgCenterY?.toFixed(2), 'px');
    console.log('Button-to-line diff:', verify.btnOnLine.toFixed(2), 'px');
    console.log('Text-to-SVG diff:', verify.textInSvg.toFixed(2), 'px');
    console.log('\nStatus:', verify.success ? '✓✓✓ PERFECTLY ALIGNED!' : '✗ Needs adjustment');

    // Screenshot after
    await page.locator('.timeline-border-container').screenshot({ path: 'top-border-after.png' });
    console.log('\n✓ Saved top-border-after.png');
    ...
    if (verify.success) {
        ...
        console.log('\n3. timeline-clean-test.css - Line ~4189:');
        console.log(`   top: ${newCssTop}px;`);
    }
