...
function setupTimelineFilters() {
    console.log('setupTimelineFilters called at', new Date().toISOString());
    ...
    setTimeout(() => {
        ...
        if (!filterButtons || filterButtons.length === 0) {
            console.error('No filter buttons found in DOM');
            return;
        }

        filterButtons.forEach((button, index) => {
            console.log(`Setting up button ${index}: "${button.textContent.trim()}" with filter="${button.dataset.filter}"`);
...
// Separate filter handler function
            ...
            btn.classList.remove('active');
                ...
                timelineMilestones.forEach(milestone => {
                    if (filter === 'all') {
                        ...
                        milestone.classList.remove('filtered-technology', 'filtered-operations');
                    } else if (filter === 'technology') {
                        if (milestone.classList.contains('technology') || milestone.dataset.category === 'technology') {
                            milestone.style.display = 'block';
                            milestone.classList.add('filtered-technology');
                        } else {
                            milestone.style.display = 'none';
                        }
                    } else if (filter === 'operations') {
                        if (milestone.classList.contains('operations') || milestone.dataset.category === 'operations') {
                            milestone.style.display = 'block';
                            milestone.classList.add('filtered-operations');
                        } else {
                            milestone.style.display = 'none';
                        }
                        ...
                        if (milestone.classList.contains('in-progress')) {
                            milestone.style.display = 'block';
                        } else {
                            milestone.style.display = 'none';
                        }
                ...
                // Also filter gallery cards to match
                galleryCards.forEach(card => {
                    if (filter === 'all') {
                        ...
                        if (card.dataset.category === 'technology' ||
                            card.dataset.id === 'wisr' ||
                            card.dataset.id === 'integrations' ||
                            card.dataset.id === 'los') {
                            card.style.opacity = '1';
                            card.style.pointerEvents = 'auto';
                        } else {
                        ...
                        if (card.dataset.category === 'operations') {
                            card.style.opacity = '1';
                            card.style.pointerEvents = 'auto';
                        } else {
                            card.style.opacity = '0.3';
                        ...
                        // For completed/in-progress/future, match based on milestone status
                        const milestoneIndex = card.dataset.index;
                        const matchingMilestone = document.querySelector(`.timeline-milestone[data-index="${milestoneIndex}"]`);
                        if (matchingMilestone && matchingMilestone.style.display !== 'none') {
                            card.style.opacity = '1';
                            card.style.pointerEvents = 'auto';
                        } else {
                            card.style.opacity = '0.3';
